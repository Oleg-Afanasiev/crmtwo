...
public class RoleDAOImpl extends GenericDAO<Role> implements RoleDAO {
    ...
    private static final String queryForGetRange = "SELECT * FROM crm.role WHERE is_deleted = FALSE ORDER BY role_id LIMIT ? offset ? ;";

    private static final String queryForInsert = "INSERT INTO crm.role (role_id) VALUES (?) RETURNING role_id ;";

    private static final String queryForUpdate = "UPDATE crm.role SET (role_id) = (?) WHERE role_id = ? ;";

    private static final String queryForGetById = "SELECT * FROM crm.role WHERE is_deleted = FALSE AND role_id = ? ;";

    private static final String queryForDelete = "UPDATE crm.role SET (is_deleted) = (TRUE) WHERE role_id = ? ;";
